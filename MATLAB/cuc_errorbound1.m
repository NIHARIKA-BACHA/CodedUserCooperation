function Pe = cuc_errorbound1() %#codegen
...
for k=1:Ni
    for m=1:Np
      ...
      Pe1 = Pe1 + ((N(k,m))*(k/Ni)*P(k,m)*(1/2* erfc(2*sqrt((k*Ec1)/(4*N0uncoded)))))/2;
    end
...
%                     mdash-ii
                ...
                for i=max(0,m+mdash-Np):min(m,mdash)%1:mdash 
                    ...
                    %if i <= m && mdash-i <= Np-m
                        sum_i = sum_i + ((nchoosek1(m, i) * nchoosek1(Np-m, mdash-i)) / sum_ii);
                        %sum_i = sum_i + ( Q_func );
...
%             semilogy(1:Np,Prob_N, 'r-*'), hold on, grid on,
            ...
            %sum_mdash
            PEP = PEP + (nchoosek1(Ni, e)* pe^e * (1-pe)^(Ni-e) *  sum_mdash * Q_func);
        end
        %PEP
